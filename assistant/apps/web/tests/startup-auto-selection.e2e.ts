...
describe('web e2e: startup auto-selection', () => {
  ...
  it('keeps the resident Hero and composer nodes when the first Workspace session appears', async () => {
    ...
    const headline = page.getByText('Trợ lý nội bộ VHD Corp', { exact: true })
    ...
    await page.evaluate(() => {
      const refs = {
        ...
        workspaceChip: document.querySelector('[aria-label="Chọn thư mục làm việc"]'),
    ...
    expect(await page.evaluate(() => {
      ...
      return {
        ...
        workspaceChip: document.querySelector('[aria-label="Chọn thư mục làm việc"]') === before.workspaceChip,
  ...
  it('keeps the hero and the composer on screen while the auto-selected blank session opens', async () => {
    ...
    expect(await page.getByText('Trợ lý nội bộ VHD Corp').isVisible()).toBe(true)
