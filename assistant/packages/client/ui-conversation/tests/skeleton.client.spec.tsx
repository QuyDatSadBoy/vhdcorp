...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    ...
    expect(view.getByText('Trợ lý nội bộ VHD Corp')).toBeTruthy()
    expect(view.getByText('Nội bộ')).toBeTruthy()
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('Trợ lý nội bộ VHD Corp')).toBeTruthy()
    expect(b.view.getByText('Nội bộ')).toBeTruthy()
    ...
    fireEvent.click(b.view.getByRole('button', { name: 'Chọn thư mục làm việc' }))
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.view.queryByText('Trợ lý nội bộ VHD Corp')).toBeNull()
  })
  ...
  it('blank session keeps the interactive picker chip (workspace switchable until the first message)', () => {
    const b = mount(conversationSnapshot({ composerPhase: 'blank', blank: true }))
    const chip = b.view.getByRole('button', { name: 'Chọn thư mục làm việc' })
