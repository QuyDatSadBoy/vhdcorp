...
- **Private-target blocking judges the hostname, not the resolved address** — `isPrivateFetchTarget` refuses `localhost`, `.local`/`.internal` names, and IP literals in the loopback, private, link-local, carrier-NAT, and unspecified ranges (IPv4, IPv6, and IPv4-mapped IPv6 forms); redirects add nothing, since only same-origin hops are followed. What remains open is DNS-resolve-then-validate: a public name answering with a private address still connects, because `fetch` does not expose the address it dialled. A deployment where that matters resolves names itself before handing the URL over, or fronts the provider with an egress proxy.
