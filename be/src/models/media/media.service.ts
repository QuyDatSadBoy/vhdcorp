...
export class MediaService {
  ...
  async uploadFile(file: Express.Multer.File, folder: string, uploadedBy: number) {
    ...
    if (useCloudinary) {
      try {
        ...
        this.logger.warn(`Cloudinary upload failed, fallback local: ${(err as Error).message}`);
      }
    }
    ...
    const apiUrl = this.config.get<string>("API_PUBLIC_URL") ?? "http://localhost:8080";
  ...
  async list(params: { pageNumber?: string; pageSize?: string; folder?: string; tag?: string }) {
