...
export class CategoryService {
  ...
  async list(params: { search?: string; parentId?: number | null; includeChildren?: boolean }) {
    ...
    return this.prisma.category.findMany({
      ...
      include: params.includeChildren ? { children: { orderBy: { order: "asc" } } } : undefined,
    });
  }
  ...
  async tree(): Promise<Category[]> {
    return this.prisma.category.findMany({
      where: { parentId: null },
      orderBy: { order: "asc" },
      include: { children: { orderBy: { order: "asc" }, include: { children: true } } },
    });
  }
  ...
  async update(id: number, dto: UpdateCategoryDto): Promise<Category> {
    ...
    if (dto.slug) slug = await this.slug.generateUniqueSlug(dto.slug, "category", id);
    else if (dto.name) slug = await this.slug.generateUniqueSlug(dto.name, "category", id);
  ...
  async delete(id: number): Promise<void> {
    ...
    if (cat.products.length > 0) {
      throw new NotFoundException("Danh mục đang có sản phẩm — hãy chuyển sản phẩm trước");
    }
