...
export default function AdminReviewsPage() {
  ...
  async function remove(id: number) {
    const ok = await confirm({
      title: "Xóa đánh giá?",
      description: "Đánh giá sẽ bị xóa vĩnh viễn khỏi sản phẩm.",
      confirmText: "Xóa",
      variant: "destructive",
    });
    if (!ok) return;
    ...
    <AdminTable
      ...
      columns={[
        ...
        { key: "content", header: "Nội dung", render: (r) => <p className="line-clamp-2 max-w-md">{r.content}</p> },
        {
          ...
          render: (r) => (
            <span
              className={`rounded-full px-2 py-0.5 text-xs ${r.status === "APPROVED" ? "bg-emerald-500/10 text-emerald-600" : r.status === "REJECTED" ? "bg-red-500/10 text-red-600" : "bg-amber-500/10 text-amber-600"}`}
            >
              {r.status}
            </span>
          ),
