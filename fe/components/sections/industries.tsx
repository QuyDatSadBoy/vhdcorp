...
const DEFAULT_ITEMS = [
  {
    ...
    description:
      "PVC, HDPE, PPR — đầy đủ kích thước D21–D200, tiêu chuẩn TCVN, đa dụng dân dụng & công nghiệp.",
  ...
  },
  {
    ...
    description:
      "Tấm cao su NBR, EPDM, SBR — gioăng, ron, đệm chống rung cho nhà máy chế biến và cơ khí chính xác.",
          ...
          {items.map((it, i) => {
            ...
            return (
              <motion.div
                ...
                className={cn(
                  "group relative overflow-hidden rounded-3xl border border-foreground/8 bg-card p-7 shadow-[0_1px_0_rgba(255,255,255,0.6)_inset,0_8px_24px_-12px_rgba(15,35,86,0.12)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_48px_-20px_rgba(15,35,86,0.30)]",
                  ACCENT_BG[accent],
                )}
              >
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div className="grid h-20 w-20 shrink-0 place-items-center rounded-2xl bg-background/70 ring-1 ring-foreground/5 backdrop-blur">
                    {it.icon ? (
                      <Image src={it.icon} alt={it.title} width={88} height={88} className="h-16 w-16 object-contain" />
                    ) : (
                      <div className="h-12 w-12 rounded-xl bg-(--vhd-color-primary)/10" />
                    )}
                  </div>
                  <span className="grid h-9 w-9 place-items-center rounded-full border border-foreground/10 bg-background/70 text-foreground/55 transition-all group-hover:border-(--vhd-color-primary)/40 group-hover:bg-(--vhd-color-primary) group-hover:text-white">
                  ...
                  <ul className="relative mt-5 space-y-2">
                    {it.bullets.map((b, idx) => (
                      <li key={idx} className="flex items-center gap-2 text-sm text-foreground/75">
                        <span className={cn("grid h-5 w-5 shrink-0 place-items-center rounded-full text-white", ACCENT_BAR[accent])}>
