...
export default function FloatingContact() {
  ...
  const c = (config?.footer as { contact?: { floatingWidget?: boolean; messengerUrl?: string; zaloUrl?: string; phone?: string; hotline?: string; email?: string } } | undefined)?.contact;
  ...
  if (c.messengerUrl) items.push({ label: "Messenger", href: c.messengerUrl, icon: MessageCircle, color: "bg-[#0084ff]" });
  if (c.zaloUrl) items.push({ label: "Zalo", href: c.zaloUrl, icon: MessageCircle, color: "bg-[#0068ff]" });
  const tel = c.hotline || c.phone;
  if (tel) items.push({ label: "Gọi ngay", href: `tel:${tel.replace(/\s+/g, "")}`, icon: Phone, color: "bg-emerald-500" });
